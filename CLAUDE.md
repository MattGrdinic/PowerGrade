...
2 Cinematic Smooth (PQ Decode) · 3 Desert Day · 4 Cinematic Landscape.
Cinematic Smooth is the ONLY preset that sets Camera (→ 11 Rec.2100 PQ, on purpose):
PQ-decoding log footage gives a compressive "wrong" transform the user loves
(near-perfect rolloff, smooth color) — discovered when the camera renumber made an old
node decode Gen 5 as PQ. It sets our Rolloff to 0 (PQ is already the shoulder).
None/Reset does NOT restore Camera.

**Built-in LUTs** (Desert Day / Cinematic Landscape presets): generated by
...
Both shipped looks were authored numerically and **user-validated on footage first try**
(2026-07-14). Full authoring process + parameter reference for adding more looks:
`docs/CREATING-LUTS.md` (LOOKS entry → regenerate → CMake copy list → optional preset
via `findLookLut()`; user plans to add more looks over time). Applied in `PowerGrade::applyPreset()` from `changedParam`, **guarded
